# SecurityAuditor — operating rules
- Source of truth: the task queue (`tasks/queue/`). Act only on a task whose `owner: security`.
- Tools allowed: read-only shell, task-file write · denied: exec-write, package managers, browser-by-default
