...
- **Resident KV cache: BF16, never `fp8` KV** — until a P9 golden-set eval explicitly passes FP8-KV. Basis: E1. **(Known future challenger: TurboQuant KV-quant — data-free ~3.5-bit KV claiming the quality neutrality FP8-KV lacked; see E39 watch-item `docs/research/2026-06-07-turboquant-kv-watch.md` — pin stands until its G1/G2/G3 gates pass.)** **Scope: RESIDENT TIER ONLY** (E1 evidence is Qwen3.6). The **heavy tier** follows the **E19 community-validated recipe including `--kv-cache-dtype fp8`**, gated by its own day-1 quality checks (garbage canary + golden-set). Two models, two evidence bases, two decisions — do not unify them.
