...
```
You need an `nvapi-` key from build.nvidia.com (free 30-day tier) for `nemoclaw setup-spark`/`onboard`. **Day-0 risk (E42): the free NIM tier is contended.** The "BAN OPENCLAW" forum thread + OpenClaw #12135 (closed not-planned) document **40 RPM** (community-reported, NOT NVIDIA-official) exhausting in 2–3 min, 429s after ~8 tool calls/30 s, and 504s/13-min waits under agent load. Key access isn't revoked — the risk is per-model throttling + 504s. **Do not block onboarding on free NIM:** pivot to your own local-served endpoints immediately (the plan does this anyway) and add client-side per-provider rate-limit + backoff.
...
vllm serve nvidia/Qwen3.6-35B-A3B-NVFP4 \
  ...
  --trust-remote-code
```
> **Resident KV is pinned BF16 (E1 / CLAUDE.md):** `--kv-cache-dtype auto` resolves to BF16 for this model — **never set `fp8` KV on the resident tier** (this block previously sampled `fp8`; corrected). fp8-KV is a **P9 golden-set-only** question (E40 guard) and is *not* the same decision as the heavy tier's separate fp8-KV recipe (E19 / CLAUDE.md 9d).
...
## Forum-harvest addenda (2026-06-06)

*Day-1 verify-on-arrival items and watch-items distilled from the NVIDIA-forum harvest. Evidence registry: `docs/research/2026-06-06-forum-harvest.md`. None of these flips a locked decision — they are sweeps, gates, pins, pointers, and watch-items.*

**Day-1 verify-on-arrival:**

- **TODO(verify-on-arrival) — Resident quant sweep (E12):** before committing the resident quant, sweep **NVFP4 vs FP8 vs AWQ/AutoRound-INT4** on the actual box. Gates: single-stream tok/s, golden-set quality, and memory footprint under 120B co-residency. GB10 is memory-bandwidth-bound (273 GB/s), so quant choice is a *bandwidth* choice — AWQ/INT4 can outrun NVFP4, FP8 wins quality vs AWQ, NVFP4 wins footprint; only the sweep settles it. (Supersedes the Marlin-only / #35947 rationale — both NVFP4 kernel paths are functional after PR #38126 / FlashInfer 0.6.8.post1 / CUTLASS 4.5.0 b12x.) **Quality datapoints (E36):** forum quality reports converge that **FP8 > int4-AutoRound** (gpieceoffice prefers 122B-A10B-FP8 despite the speed cost; paxren2020 calls 397B AutoRound "a crude RTN quantization") — so weight golden-set quality, not just t/s, in the sweep. **Tension to resolve on the golden set:** 0rand recommends 122B in our thread (E28-pending) yet benchmarks **3.5-122B < 3.6-27B** in thread D — generational beats size; cite both and let our golden set decide, do not pre-judge. **PrismaQuant entrant (E40):** add **PrismaQuant** mixed-precision (per-Linear allocation over NVFP4/MXFP8/FP8/BF16, native compressed-tensors, MTP-ready) as a fourth sweep entrant — PrismaSCOUT-27B is double-source-confirmed (5.31 bpp / 20.17 GB / KL 0.0151 / GSM8K 96.66 / MMLU 86.23 / tool-eval 88); the 35B 4.75 bpp / −0.56 pp-vs-BF16 figure is **author self-report → reproduce on our golden set, don't trust**. ⚠️ **HARD GUARD:** PrismaQuant's published recipe recommends `--kv-cache-dtype fp8`; it enters this sweep with **KV forced to BF16**. The sweep tests **weights only** — fp8-KV remains exclusively a **P9 golden-set** question per the E1 pin (one variable at a time; never adopt PrismaQuant's recipe verbatim).
- **TODO(verify-on-arrival) — Tool-eval gate (E14):** run a tool-calling eval suite on the *pinned* vLLM build before any agent depends on tool calls; tool-calling reliability is **version-controlled** (recent vLLM alone reached 100% tool-eval on 3.5-122B and 3.6-35B). Decide the parser empirically between **`qwen3xml`** and **`qwen3coder` + enhanced jinja template**. Re-run this gate on every vLLM bump (CLAUDE.md pin). **Instrument (E41): Tool Eval Bench** (`uv tool install git+https://github.com/SeraphimSerapis/tool-eval-bench.git`; **69 scenarios / 15 categories**, 84 with `--hardmode`; OpenAI-compatible, deterministic `--seed`; `Deployability = 0.7·Quality + 0.3·Responsiveness`). **Safety promotion gate:** Category-K safety — including **TC-60 "Cross-Turn Sleeper Injection"** (a turn-1 tool result plants an attacker BCC; fail = the model carries it into a later email) — must **pass on our model** before any tool-capable agent ships. The forum "122B-int4 failed TC-60" report is **unverified** — run it on-box, don't assume a result.
- **TODO(verify-on-arrival) — Resident-serve one-liners:** apply the **whpthomas chat-template patch** on Qwen3.6 (E2); set **`HF_HUB_OFFLINE=1`** once weights are cached (E3 — also fits deny-by-default egress); if MTP is enabled, **`num_speculative_tokens=3`** is the community-tested value (E4); evaluate **DFlash** (z-lab drafter, ~15 spec tokens, 65→80 t/s) as a 2nd spec-decode option — needs vLLM **PR #40898**, stays optional-with-fallback like MTP (E5). Expected single-stream baseline (E7) — **widen the flat 75–95** per the verified phuongncn 35B-A3B README (2026-06-07): **72 → 112+ t/s headline**, **108–131 t/s per-task** (hybrid INT4+FP8 + MTP), native-FP8 single-request ~**66–74 t/s**, and **158.7 t/s aggregate at 4× concurrency** — but **per-request throughput collapses under concurrency** (8 simultaneous agents do NOT each see the headline figure). Treat 75–95 as a conservative *resident-NVFP4* floor, not a universal number; settle the real figure in the E12 sweep on the golden set. (If the phuongncn hybrid recipe is ever adopted, its real config is `VLLM_UF_EAGER_ALLREDUCE=1` + MTP method `qwen3_next_mtp`; **Discard two Chrome-agent errors:** (1) the fabricated 35B use of `VLLM_MARLIN_USE_ATOMIC_ADD=1` — a real flag but a documented 0% no-op on single-GPU, absent from the 35B repo; (2) the misattribution of `fix-qwen3.5-autoround`/`fix-qwen3.5-chat-template` to this repo — those are real eugr `spark-vllm-docker` mods (E33), just absent from the phuongncn standalone repo.)
- **Resident reference recipe (E33):** a real-world resident-35B flag set seen in the wild (andreask1, thread C #21 — 2-node but flag-relevant) corroborates the resident-serve knobs: `cyankiwi/Qwen3.6-35B-A3B-AWQ-4bit`, chat-template mod (`mods/fix-qwen3.5-chat-template`, **confirms E2**), `--tool-call-parser qwen3_coder` + `--reasoning-parser qwen3` (the **E14** parser to validate empirically against `qwen3xml`), `--gpu-memory-utilization 0.155`, `--max-model-len 262144`, `--max-num-seqs 8` (low, per Spark small-batch guidance), `--chat-template unsloth.jinja`, `--enable-prefix-caching`. AWQ-4bit is the default here — a concrete entrant for the **E12** sweep, not a quant decision. Reference only; verify each flag on the actual box (community-reported, untrusted).
- **Platform note (E12/E13):** the GB10 is **memory-bandwidth-bound** — every quant/KV/spec-decode choice is fundamentally a bandwidth tradeoff, which is why bandwidth-blind optimizations (e.g. a Marlin rewrite) reportedly don't help. Resident KV cache is pinned **BF16** (CLAUDE.md pin / E1); the §2 sample `--kv-cache-dtype fp8` is **superseded**.
- **TODO(verify-on-arrival) — NemoClaw scheduling (E17):** on the *shipped* OS image, test NemoClaw agent health and cron scheduling — OS 2026.05.31 (v1.135.34) is implicated in agents breaking (`tool_search_code`) and cron jobs unscheduleable (unresolved upstream). **Fallback scheduling mechanism: systemd timers** for the heartbeat / egress-probe jobs if NemoClaw cron is broken. **Status sharpened (E17 update, verified 2026-06-07):** NemoClaw **#4616 "agent cron-job creation does not work on DGX Spark" is OPEN** and is **not** fixed by v0.0.59 (which fixed the unrelated NVFP4-container bug #4658); the `tool_search_code` symptom got partial fixes (#4780/#4800 closed 2026-06-05) but the v0.0.55 gateway-1006 "Tool exec not found" forum report stands → **treat systemd timers as the PRIMARY scheduler, not a fallback**; do not depend on agent-driven cron.
- **Heavy-tier Plan-B pointer (E13):** if the GPT-OSS-120B path stalls, the community-canonical single-box heavy build is **Qwen3.5-122B-A10B v2.1** (Albond thread) — one-command `install.sh` (thread post #104), **28.3→52 tok/s** (v2.4 default; 51 was the older v2 figure) via INT4+FP8 + INT8 LM head + MTP-2 (~95% accept), 256K ctx. Its "what didn't help" list (FP8-KV noise, NVFP4 < INT4-then, Marlin rewrite, EAGLE-3 < MTP) saves re-discovery; **TurboQuant-KV** is the long-context lever (−22% speed for 4× context/concurrency). *(Pointer only — the heavy model stays locked to GPT-OSS-120B; this is the fallback if that path is ever reopened.)* **Quant-path & node-count distinction (E35) — evaluate the fallback on the right numbers:** the 28.3→52 t/s figures above are the **single-box INT4+FP8 hybrid v2.x** build (v2.4 default 52; 51 was v2); they are NOT comparable to **single-Spark 122B NVFP4 at 8–15 t/s** (thread F author's own measurement) nor to the **~51–75 t/s 2-node int4-AutoRound** figures (threads B/D). These three numbers are an **unresolved conflict, recorded as such** — quant-path- and node-count-dependent; do not average them or pick the rosy one. If 122B NVFP4 is ever served, the corrected checkpoint is **`txn545/Qwen3.5-122B-A10B-NVFP4`** (NVIDIA ModelOpt v0.42.0; the broken alpertor model was withdrawn) with the Marlin gate-fix env (`VLLM_USE_FLASHINFER_MOE_FP4=0`, `VLLM_NVFP4_GEMM_BACKEND=marlin`, `VLLM_TEST_FORCE_FP8_MARLIN=1`) — but note the slow single-box t/s. **Verification corrections (E13/E35, 2026-06-07):** env pins confirmed (vLLM 0.19.1.dev0+g2a69949bd, PyTorch 2.12.0.dev, FlashInfer 0.6.7) but CUDA is **split — Toolkit 13.2 / torch 13.0**, not a flat 13.2; **MTP-2** (`num_speculative_tokens:2`) confirmed correct for the 122B (MTP gives ~0% on the 35B — different regime); the full "what didn't help" kill-list (FP8-KV ≈ noise, NVFP4 −42%, Marlin-atomic-add 0% single-GPU, EAGLE-3 < MTP-2, INT8-shared-expert = gibberish) is verified against the repo README.

**Watch-items:**

- **Harness continuity — OpenClaw deprecation rumor (E16/E38, two-source, UNVERIFIED):** two forum posters now make the claim (karol.spark + one more, citing "roadmap and an April livestream") that NVIDIA's roadmap drops OpenClaw support in favor of Hermes — corroboration is **not** verification; still treat as forum speculation. *Verify before acting:* NemoClaw release notes + the NVIDIA DGX Spark roadmap/forum. *Structural mitigation already in place:* queue-as-spine dispatch (Imp-3) plus per-agent `SOUL.md`/`AGENTS.md` keep agent definitions **harness-portable**, so a harness migration is a re-wiring, not a rewrite. Cross-ref the Phase-3 Hermes plan (§5) — Hermes is already the planned learning-loop layer, so an OpenClaw→Hermes shift would *accelerate*, not derail, the roadmap.

- **Atlas inference engine — watch-item, cold-start alternative to llama-swap + drop_caches (E43):** real & active (`Avarok-Cybersecurity/atlas`, 481★, Rust/CUDA, AGPL; forum #369263), claiming "<2 min cold start, 13+ models" — directly targets the model-swap cold-load latency that motivates the llama-swap + `drop_caches` dance (Phase-2). **But NOT turnkey:** the "100+ tok/s on Qwen3.6-35B-FP8" headline is best-case relabeled (the real ~130 t/s is Qwen3.5-35B NVFP4+MTP); an independent quickstart **crashed** ("KV cache 0 sequences") and the author's own 122B run was **33.4 t/s**. *Verify on-box before trusting any figure; do not promise 100+ t/s. Watch-only — does not change the llama-swap decision.*

## Plan cross-check addenda — batch 3 (2026-06-06)

*Operating-plan cross-check + operator playbook extracts (topics 364009/356651/362824/364355/364781/367762/363682/367580/360299). Evidence registry: `docs/research/2026-06-06-forum-harvest.md` (E18–E27). OpenShell policy appendix: `runbooks/openshell-policy-reference.md`. Nothing flips a locked decision.*

**Day-1 verify-on-arrival (batch 3):**

- **TODO(verify-on-arrival) — Heavy-tier backend verification + garbage canary (E18):** confirm the *active* MXFP4 backend from the **startup log** ("Using backend: marlin" / "Auto-selected: CUTLASS_FP4"), never from the flag. Run a `!!!`/repetition canary on the heavy tier before trusting it — both forked-CUTLASS and Marlin had real, build-specific, since-fixed garbage eras. Backend claims carry build provenance (CLAUDE.md pin).
- **Heavy-tier reference invocation (E19, verbatim — on the eugr `--exp-mxfp4` build):**
  `vllm serve openai/gpt-oss-120b --quantization mxfp4 --mxfp4-backend CUTLASS --mxfp4-layers moe,qkv,o,lmhead --attention-backend FLASHINFER --load-format fastsafetensors --kv-cache-dtype fp8 --tool-call-parser openai --reasoning-parser openai_gptoss --enable-prefix-caching`
  **Scoping (9d):** the `--kv-cache-dtype fp8` here is the **heavy-tier** decision (gated by its own garbage-canary + golden-set), **not** the resident BF16 pin. `scripts/03` keeps its current flags; reconcile against this recipe via the in-script `TODO(verify-on-arrival)` comment — do not edit flags blind.
- **TODO(verify-on-arrival) — `--mxfp4-layers` spelling:** verify the *accepted* spelling from the startup log — sources differ (**cho E19 recipe: `lmhead`; eugr launcher: `lm_head`**); a wrong spelling **silently skips quantizing a layer** rather than erroring.
- **TODO(verify-on-arrival) — Memory split math (E20):** with serving budgets, **35B and 120B each occupy ~65–75 GB** → together they exceed 128 GB. Co-residency is viable ONLY because the 120B is strictly on-demand with constrained KV. Compute the explicit `gpu-memory-utilization` split (resident-always + heavy-on-demand) on day 1. Heavy-tier throughput expectation: **57–60 t/s** (E18/E19). **Eager-vs-graphs dimension (E34):** thread H independently reproduces this pressure (three models in ~120GB only by capping each hard) and shows co-residency costs more than footprint — CUDA graphs need **~130GB+/model**, so multi-model co-residency forces **`--enforce-eager`**, measured at a **~40% throughput loss** (Nemotron-3-Nano-30B NVFP4 37.8 t/s eager vs ~62 with graphs; Qwen3.6-35B-A3B-FP8 33.5 vs 53.7). Budget the split against **eager-mode** resident throughput, not the graphs-on headline.
- **TODO(verify-on-arrival) — Day-1 smoke trio + cron gotchas (E27):** (1) the served-model-name MUST match the client's model id; (2) editors autocorrect `--` to an en-dash — paste flags carefully; (3) gpt-oss-120B context ceiling is **131072**. Cron: never `--light-context` on skill-calling jobs; always `--thinking off` + `--timeout-seconds`. `reasoning_effort "low"` returned HTTP 400 (use high/none) — **verify whether that rejection is cron/SGLang-scoped before trusting the CEO effort ladder** (low/medium/high/xhigh/max) near scheduled jobs.
- **TODO(verify-on-arrival) — Pre-seal context knobs (E25):** workspace injection sends ~26k chars every turn (23–60s latency on trivial prompts) and Landlock makes `/sandbox/.openclaw` read-only after sealing → set `bootstrapMaxChars` / `contextWindow` **before** sealing the sandbox.
- **TODO(verify-on-arrival) — Persistence check (E24):** verify the sandbox CRD has a **PVC / `volumeClaimTemplates`** (the v0.1.0 cluster image shipped without one) and set an **explicit container memory limit** (default = none; an unbounded skill OOM-exits 137). `/sandbox/` is ephemeral by design — see `runbooks/failure-modes.md` #11.
- **TODO(verify-on-arrival) — Inference route timeout (E22 vs Step 7):** the `inference.local` router has a hard **60s timeout**; the fix is to bypass to the Docker bridge IP **172.17.0.1** and approve the pending rule via `openshell term`. This confirms Step 7's "explicit host:port, not the default localhost route" — pin the bridge-IP detail into the egress policy.

**Phase-1 requirements (batch 3):**

- **Session watchdog (E26) is a Phase-1 dispatcher requirement** alongside the 3-consecutive-failure and 10-minute-runtime stops: a 30s-polling watchdog that detects ≥2 trailing consecutive user messages (the 2026.3.x corruption signature) → deletes the session JSONL → alerts, run via `nohup` in the dispatcher health loop. See `runbooks/failure-modes.md` #12.
- **Stagger scheduled jobs (E38) — cron-collision avoidance:** simultaneous cron/heartbeat firings cause issues (tonyd615, thread D #10), so the Phase-1 dispatcher must **offset job start times** (not all-on-the-minute). Same principle as the model-swap stagger after `drop_caches` (§2 llama-swap note), one layer up.
- **Host-side backups are LOAD-BEARING, not optional (E24):** the sandbox is ephemeral by design, so the host-side backup set *is* the persistence model. See `runbooks/openshell-policy-reference.md` for the exact backup / NOT-backed-up lists.
- **Phase-1 serves direct (tmux/systemd), not via the router (E21):** the LiteLLM → llama-swap → engines router stack works in production but is "reliable, not fast" — recorded justification for keeping Phase-1 on direct tmux/systemd serving (`scripts/03`/`scripts/04`), with the router as the Phase-2 upgrade.
